...
class OnRobotRGNode(Node):
    ...
    def __init__(self, nodeName : str):
        ...
        self._last_status_error_log_time = 0.0
        ...
        try:
            initial_status = self.gripper.getStatus()
            self.command.rgwd = int(
                min(self.max_width, initial_status['width'])
            )
        except Exception as error:
            # Keep the node alive when the gripper is disconnected at startup.
            # The periodic status reader will retry and resume publication after
            # the Modbus connection returns.
            self.command.rgwd = int(self.max_width)
            self._report_status_error(error)
    ...
    def initParams(self):
        ...
        self.grip_detected_pub = self.create_publisher(Bool, '/onrobot/grip_detected', 3, callback_group=self.reentrant)
    ...
    def _report_status_error(self, error):
        now = self.get_clock().now().nanoseconds / 1.0e9
        error_text = f"{type(error).__name__}: {error}"
        if (
            error_text != self._last_status_error
            or now - self._last_status_error_log_time >= 2.0
        ):
            self.get_logger().error(
                "RG2 status read failed; healthy status publication is "
                f"paused: {error_text}"
            )
            self._last_status_error_log_time = now
        self._last_status_error = error_text

    def getStatus(self):
        try:
            self.status = self.gripper.getStatus()
        except Exception as error:
            # Do not replay the last healthy sample.  Consumers detect the
            # missing heartbeat as a signal loss, while this timer keeps
            # retrying so a cable reconnection can recover automatically.
            self._report_status_error(error)
            return

        if self._last_status_error:
            self.get_logger().info("RG2 status communication recovered")
            self._last_status_error = ""

        status_msg = OnRobotRGInput()
        status_msg.gfof = int(
            self.status.get('gfof_raw', round(self.status['offset'] * 10.0))
        ) & 0xFFFF
        status_msg.ggwd = int(
            self.status.get(
                'ggwd_raw',
                round(self.status['relative_width'] * 10.0),
            )
        ) & 0xFFFF
        status_msg.gsta = int(
            self.status.get('gsta', self.status['busy'])
        ) & 0xFF
        status_msg.gwdf = int(
            self.status.get('gwdf_raw', round(self.status['width'] * 10.0))
        ) & 0xFFFF
        ...
        # gSTA(holding register 268)는 self.status['busy']에 원시 비트값으로
        # 보관된다. README 계약에 따라 bit 1을 실제 물체 파지 신호로 노출한다.
        grip_msg = Bool()
        grip_msg.data = bool((int(status_msg.gsta) >> 1) & 0x1)
