...
class communication(OnRobotCommunicationBase):
    ...
    def getStatus(self):
        ...
        if not self.client.connected and not self.client.connect():
            raise ConnectionError("OnRobot Modbus TCP reconnect failed")
        ...
        with self.lock:
            ...
            if result is None or result.isError():
                ...
                raise ConnectionError(
                    f"OnRobot Modbus status read failed: {result!r}"
                )
            response = getattr(result, "registers", None)
            if response is None or len(response) < 18:
                self.client.close()
                raise ConnectionError(
                    "OnRobot Modbus status response is incomplete: "
                    f"{response!r}"
                )
        # Output the result

        return {
            # Keep the raw Modbus words for the ROS status message.  In
            # particular register 268 (gSTA) contains all RG2 safety bits.
            'gfof_raw': response[0],
            'ggwd_raw': response[9],
            'gsta': response[10],
            'gwdf_raw': response[17],
